  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-10">
          ...
          <div className="flex flex-col gap-3 max-w-xs">
            <a
              ...
              className="text-[18px] font-extrabold tracking-[0.1em] text-white hover:text-[#9EC49E] transition-colors cursor-pointer"
            >
              {personal.name.toUpperCase()}
            </a>
            <p className="text-[12px] text-[#8FB88F] leading-relaxed font-normal">
              {personal.subRole}.<br />
              {personal.subTagline}
            </p>
          </div>
            ...
            <nav className="flex flex-wrap gap-6 md:gap-8 justify-start md:justify-end">
              {footerNav.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-[13px] text-white/85 hover:text-white transition-colors font-normal"
                >
                  {link.name}
                </a>
              ))}
      ...
      <div className="max-w-5xl mx-auto px-6 border-t border-white/15 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
        <p className="text-[11px] text-[#6A9A6A]">
          © {new Date().getFullYear()} {personal.name}. All rights reserved.
        </p>
        <a
          href={`mailto:${personal.email}`}
          className="text-[11px] text-[#6A9A6A] hover:text-white transition-colors"
        >
          {personal.email}
        </a>
      </div>
